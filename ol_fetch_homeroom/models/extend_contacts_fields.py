...
class SchoolStudent(models.Model):
    ...
    def get_grade_level_and_custom_defined_fields_for_addmissions(self):
        school_name_key = {
        ...
        students = self.env['school.student'].search([('x_studio_batch_proceed', '=', False), ('x_last_enrollment_status_id.name', 'in', ['Admission'])], limit=10)
        for std in students:
            api_key = school_name_key.get(std.x_last_school_id.name)
            headers['Facts-Api-Key'] = api_key
            ...
            response = requests.request("GET", url, headers=headers).json()
            if response.get('results') and len(response.get('results')) > 0:
                std.x_studio_father_cnic = response.get('results')[0].get('data')
            
            # Mother CNIC
            url = f"https://api.factsmgt.com/UserDefinedData?Page=1&api-version=1&filters=linkedId=={std.facts_id},fieldId=={mother_cnic_id}"
            response = requests.request("GET", url, headers=headers).json()
            if response.get('results') and len(response.get('results')) > 0:
                std.x_studio_mother_cnic = response.get('results')[0].get('data')
    ...
    def get_homeroom_and_custom_defined_fields(self, timeline='daily', student_id=None):
        ...
        if not student_id:
            if timeline == 'daily':
                students = self.env['school.student'].search([('x_studio_batch_proceed', '=', False), ('homeroom', '=', False), ('x_last_enrollment_status_id.name', 'in', ['Enrolled'])], limit=10)
            else:
