...
class MakeStudentCharge(models.TransientModel):
    ...
    split_by_student = fields.Boolean(
        string='Split by student', default=lambda self: self.env.company.ol_school_account_split_by_student)
...
class MakeStudentChargeLine(models.TransientModel):
    ...
    def _get_pricelist_data(self, student, family=False):
        ...
        date = fields.Date.today()
        company = self.company_id
        return self.env['ol.finance.util'].get_pricelist_price_data(pricelist, product, quantity, product_uom, partner, date, company)
