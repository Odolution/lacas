...
class ext(models.Model):
    ...
    class_sec=fields.Char(string="Class",compute="_onchange_class_sec_data")
    sec_class=fields.Char(string="Section",compute="_onchange_sec_class_data")
...
#         return action  
    ...
    @api.onchange('student_ids')
    def _onchange_sec_class_data(self):
        self._get_sec_class_field()
        ...
        for rec in monthly_bill:
            if rec.student_ids:
                ...
                if len(splitted_name)>2:
                    rec.class_sec=splitted_name[0]+"-"+splitted_name[1]
                   
                elif len(splitted_name)>1:
                    rec.class_sec=splitted_name[0]
                   
                elif len(splitted_name)>0:
                    rec.class_sec=splitted_name[0]
                

    def _get_sec_class_field(self):
        monthly_journal=self.env['account.journal'].search([('code','=','MNT')])
        monthly_bill=self.env['account.move'].search([('journal_id','=',monthly_journal.id)])
        self.sec_class=' '
        for rec in monthly_bill:
            if rec.student_ids:
                ...
                splitted_name=classnsec.split('-')
                if len(splitted_name)>2:
                    rec.sec_class=splitted_name[2]
                elif len(splitted_name)>1:
                    rec.sec_class=splitted_name[1]
