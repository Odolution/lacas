...
class Account_move(models.Model):
    ...
    def _compute_withdrawn_status_bill(self):
        for rec in self:
            if rec['move_type']=='out_invoice' and rec['std_olfid']:
                reversal= self.env['account.move'].search([('move_type','=','out_refund'), ('olf_id_cred_custom','=',rec['std_olfid']),('withdrawn_status_reversal','=','Y'),('state','=','posted')])
                if reversal:
