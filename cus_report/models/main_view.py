...
class reportbutton(http.Controller):
    ...
    def download_catalogue(self, **kw):
        ...
        pdf, _ = request.env.ref('cus_report.report_fee_challan_student_wise').sudo()._render_qweb_pdf(
            [int(record_id)])
    ...
    def _compute_unpaid_inv_students(self):
        ...
        for rec in self:
            if rec.move_type=='out_refund':
                ...
                rec.unpaid_std_inv_ids=self.env['account.move'].search([("move_type","=","out_invoice"),("student_ids","=",stud.id),("payment_state","=","not_paid"),('journal_id','in',[125,126])])       
