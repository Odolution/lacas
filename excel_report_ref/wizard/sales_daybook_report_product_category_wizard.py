...
class sale_day_book_wizard(models.TransientModel):
    ...
    def _compute_quantities_product_quant_dic(self,lot_id, owner_id, package_id,from_date,to_date,product_obj,data):
        ...
        # raise UserError(str(custom_domain))
        ...
        # raise UserError(str(domain_quant))
        if dates_in_the_past:
        ...
        for product in product_obj.with_context(prefetch_fields=False):
            ...
            res[product_id] = {}
            if dates_in_the_past:
                # raise UserError(str(quants_res.get(product_id, 0.0)))
    ...
    def get_lines(self, data):
            ...
            for product in  product_res :
                ...
                stock_internal_lines = self.env['stock.move'].search([('location_id.usage', '=', 'internal'),('location_dest_id.usage', '=', 'internal'),('product_id.id','=',product.id),('state', '=', 'done')] + inventory_domain)
    ...
    def print_exl_report(self):
        if xlwt:
            ...
            if self.display_sum:
                ...
                worksheet.write(8, 1, 'Name test', style_table_header)
                worksheet.write(8, 2, 'Uom', style_table_header)
                worksheet.write(8, 3, 'Category', style_table_header)
