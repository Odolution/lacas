...
class RecoveryReportWizard(models.TransientModel):
    ...
    def action_print_excel_billing_report(self):
        if xlwt:
            ...
            worksheet.write_merge(0,1,0,1, 'S#',  style=red_style_title)
            worksheet.write_merge(0,1,2,4, 'Branch',  style=red_style_title)
            worksheet.write_merge(0,1,5,7, 'Total Issuance',  style=red_style_title)
            worksheet.write_merge(0,1,8,10, 'Net Billing Exc.Withdrawals',  style=red_style_title)
            worksheet.write_merge(0,1,11,13, 'Total Recovery',  style=red_style_title)
            worksheet.write_merge(0,1,14,16, 'Receivables',  style=red_style_title)
            worksheet.write_merge(0,1,17,19, 'Bade Dabts',  style=red_style_title)
            worksheet.write_merge(0,1,20,22, "'%'age of Recovery on Enrolled and Paid Bills",  style=red_style_title)
            worksheet.write_merge(0,1,23,25, "Actual Recovery '%'age",  style=red_style_title)
            # worksheet.(0,1,0,3,"",)
